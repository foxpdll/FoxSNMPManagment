...
func checkIfaces(w http.ResponseWriter, r *http.Request){
    ...
    for i := range snmp_coms_r{
        ...
        if err == nil{
            ...
            fmt.Fprintf(w,"<tr><th>SwitchMAC</th><th>ifId</th><th>ifAdminStatus</th><th>ifOperStatus</th><th>ifName</th><th>ifAlias</th><th>ifSpeed</th><th>ifType</th><th>ifMAC</th><th>ifInErr</th><th>Mcast</th><th>LLDP&FDB Table</th></tr>")
...
//              fmt.Fprintf(w,"</td>")
                ...
                for j := range lldpchasid {
                    if fmt.Sprintf("%d",i+1) == strings.Split(lldpchasid[j].Name,".")[13] {
                        ...
                        fmt.Fprintf(w,"<tr><td>SystemName</td><td>%s</td></tr></table>",lldpsysname[j].Value)
            ...
            fmt.Fprintf(w,"</td></tr></table></body></html>")
            return
        }
    }
...
func setSnmpStr(ip string, oid string, val string){
    for i := range snmp_coms_w {
        ...
        pdus = append(pdus, gosnmp2.SnmpPDU{oid, 4, val})
...
func setSnmpInt(ip string, oid string, val int){
    for i := range snmp_coms_w {
        ...
        defer gosnmp2.Default.Conn.Close()
        var pdus []gosnmp2.SnmpPDU
        pdus = append(pdus, gosnmp2.SnmpPDU{oid, 2, val})
        gosnmp2.Default.Set(pdus)
        gosnmp2.Default.Conn.Close()
    }
}
